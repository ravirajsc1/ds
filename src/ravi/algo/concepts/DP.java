...
public class DP {
    ...
    public static int findMaxKnapsackProfit(int capacity, int [] weights, int [] values) {
        ...
        PriorityQueue<double []> maxHeap=new PriorityQueue<>((a,b)-> (int) (b[0]-a[0]));

        for(int i=0;i<weights.length;i++){
            maxHeap.add( new double[]{ values[i]/weights[i],i});
        }

        int weightSum=0;
        int maxValue=0;
        while(!maxHeap.isEmpty()){
            double[] sacks=maxHeap.poll();
            //double weight=sacks[0];
            int index=(int)sacks[1];
